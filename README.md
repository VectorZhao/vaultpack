...
GitHub Actions 会在发布 GitHub Release 时构建并推送多架构镜像。Release tag 使用 `v1.1.0` 这类格式时，Docker 镜像会自动去掉开头的 `v`，发布为 `1.1.0`。
...
例如发布 `v1.1.0` 时，推送的标签为 `vectorzhao/vaultpack:1.1.0` 和 `vectorzhao/vaultpack:latest`。
