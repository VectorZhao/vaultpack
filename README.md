...
GitHub Actions 会在发布 GitHub Release 时构建并推送多架构镜像。Release tag 使用 `v2.1.9` 这类格式时，Docker 镜像会自动去掉开头的 `v`，发布为 `2.1.9`。
...
例如发布 `v2.1.9` 时，推送的标签为 `vectorzhao/vaultpack:2.1.9` 和 `vectorzhao/vaultpack:latest`。
