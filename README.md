...
| PC 日间模式 | PC 夜间模式 |
| --- | --- |
| ![vaultpack PC 日间模式](docs/screenshots/pc-light.png) | ![vaultpack PC 夜间模式](docs/screenshots/pc-dark.png) |

| 移动端日间模式 | 移动端夜间模式 |
| --- | --- |
| ![vaultpack 移动端日间模式](docs/screenshots/mobile-light.png) | ![vaultpack 移动端夜间模式](docs/screenshots/mobile-dark.png) |
...
GitHub Actions 会在发布 GitHub Release 时构建并推送多架构镜像。Release tag 使用 `v2.1.10` 这类格式时，Docker 镜像会自动去掉开头的 `v`，发布为 `2.1.10`。
...
例如发布 `v2.1.10` 时，推送的标签为 `vectorzhao/vaultpack:2.1.10` 和 `vectorzhao/vaultpack:latest`。
